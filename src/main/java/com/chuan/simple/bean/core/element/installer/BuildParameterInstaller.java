...
public class BuildParameterInstaller
        extends ParameterInstaller<Builder<?>, BuildParameterElement> {

    @Override
    public void install(Builder<?> builder,
            List<BuildParameterElement> elements, String period) {
        if (!StringHelper.equals(period, PERIOD_BEFORE_CREATE)) {
            return;
        }
        List<Element> params = builder.getBuildParameters();
        parseParameters(elements, params);
    }
